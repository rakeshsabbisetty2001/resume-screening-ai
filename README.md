...
- **Body size limit:** requests are fully buffered and capped (300KB — raised from an initial 32KB once real PDF/DOCX resumes started exceeding it, see Known limitations) before Pydantic validation ever runs, not just field-level `max_length` — a large body can OOM a container before validation gets a chance to reject it.
...
- **No legacy .doc support.** The UI accepts `.txt`/`.pdf`/`.docx` (text extracted client-side in the UI via `pypdf`/`python-docx`, never on the API — see Architecture). The old binary `.doc` format (pre-Word 2007) isn't parseable by `python-docx` at all, and pulling in something that can read it (a system binary like antiword) is a lot of deploy-fragile weight for a format that's been obsolete for nearly two decades. A scanned/image-only PDF with no text layer is rejected with a clear per-file error rather than silently scoring an empty resume. A very long or heavily-formatted resume (extracted text over 100,000 characters — several dozen pages' worth) hits the API's field-length cap and is rejected with a validation error rather than truncated silently.
