...
def noise_floor(unswapped_scores: list[float]) -> dict:
    return {"mean": mean(unswapped_scores), "stdev": pstdev(unswapped_scores) if len(unswapped_scores) > 1 else 0.0}
...
def delta_beyond_noise(variant_a_scores: list[float], variant_b_scores: list[float],
    ...
    threshold = max(2 * floor["stdev"], 0.05)  # floor a nonzero threshold even if stdev happens to be 0
    return {
